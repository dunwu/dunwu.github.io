...
**DDL**，英文叫做 Data Definition Language，即**『数据定义语言』**。**DDL 用于定义数据库对象**，定义操作包括了：创建、删除、修改；而被操作的对象包括：数据库、数据表和列、视图、索引。

DDL 的核心指令是 `CREATE`、`ALTER`、`DROP`。

##### DML

**DML**，英文叫做 Data Manipulation Language，即**『数据操作语言』**。**DML 用于访问数据库的数据**，访问操作包括增加、删除、修改。

DML 的核心指令是 `INSERT`、`UPDATE`、`DELETE`、`SELECT`。这四个指令合称 **CRUD**，英文单词为 Create, Read, Update, Delete，即增删改查。
