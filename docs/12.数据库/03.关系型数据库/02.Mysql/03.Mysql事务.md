...
下面，通过一个示例来解释版本链，假设有一张表 `tx_isolation_demo`，结构如下：

```sql
CREATE TABLE `tx_isolation_demo` (
	`id` INT(11) NOT NULL,
...
接下来，我们向这张表中提交四条更新 SQL：

```sql
INSERT INTO `tx_isolation_demo` VALUES (1, 1);

UPDATE `tx_isolation_demo`
SET `value`= 2
WHERE `id` = 1;
